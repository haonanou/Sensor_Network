...
var ThunderboardReactServiceEnv = function (noble_char_list) {
  ...
  noble_char_list.forEach((c) => {
    ...
    } else if (c.uuid === "c8546913bfd945eb8dde9f8754f4a32e") {
      // ambient light
      this._noble_char_l = c;
    } else if (c.uuid === "c8546913bf0245eb8dde9f8754f4a32e") {
      // sound pressure level
      this._noble_char_s = c;
    } else {
      // console.log(c.uuid);
    }
  });
...
ThunderboardReactServiceEnv.prototype._parseResponse = function (
  char_uuid,
  buf
) {
  if (!buf) {
    return null;
  } else if (char_uuid === "2a6f") {
    // humidity
    return { humidity: buf.readUInt16LE(0) / 100 };
  } else if (char_uuid === "2a6e") {
    // Temperature
    return { temperature: buf.readInt16LE(0) / 100 };
  } else if (char_uuid === "2a76") {
    // UV Index
    return { uvIndex: buf.readUInt8(0) };
  } else if (char_uuid === "2a6d") {
    // Pressure
    return { pressure: buf.readUInt32LE(0) / 1000 };
  } else if (char_uuid === "c8546913bfd945eb8dde9f8754f4a32e") {
    // Ambient light
    return { light: buf.readUInt32LE(0) / 100 };
  } else if (char_uuid === "c8546913bf0245eb8dde9f8754f4a32e") {
    return { sound: buf.readInt16LE(0) / 100 };
  } else {
    return null;
  }
};

ThunderboardReactServiceEnv.prototype._readCharacteristic = function (
  noble_char,
  callback
) {
  noble_char.read((error, buf) => {
    ...
    if (error || !res) {
      var err = new Error(
        "Failed to get the characteristics: " + (error ? error.toString() : "")
      );
      callback(err);
    } else {
      callback(null, res);
...
/* ------------------------------------------------------------------
 * Method: getPressure(callback)
 * ---------------------------------------------------------------- */
ThunderboardReactServiceEnv.prototype.getPressure = function (callback) {
  this._readCharacteristic(this._noble_char_p, callback);
};

/* ------------------------------------------------------------------
 * Method: getAmbientLight(callback)
 * ---------------------------------------------------------------- */
ThunderboardReactServiceEnv.prototype.getLight = function (callback) {
  this._readCharacteristic(this._noble_char_l, callback);
};

/* ------------------------------------------------------------------
 * Method: getSoundLevel(callback)
 * ---------------------------------------------------------------- */
ThunderboardReactServiceEnv.prototype.getSoundLevel = function (callback) {
  this._readCharacteristic(this._noble_char_s, callback);
};
...
ThunderboardReactServiceEnv.prototype._readCharacteristics = function (
  noble_char_list,
  callback
) {
